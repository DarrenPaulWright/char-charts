...
import type { IBandDomain } from '../types';
...
describe('init', () => {
	...
	it('should set the domain and maxLabelWidth', () => {
		...
		const domain = scale.domain as Array<IBandDomain>;
		...
		domain.forEach((item, index) => {
			assert.is(
				item.siblings[0],
				index === 0 ? undefined : domain[index - 1]
			);
			assert.is(
				item.siblings[1],
				index === domain.length - 1 ? undefined : domain[index + 1]
			);
		});
